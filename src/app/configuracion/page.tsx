  ...
  async function handleSave() {
    ...
    try {
      for (const tasa of tasas) {
        const nuevaTasa = parseFloat(
          editTasas[tasa.moneda] || tasa.tasa.toString(),
        );
      ...
      setMensaje({
        tipo: "success",
        texto: "Tasas actualizadas correctamente",
      });
    } catch (error) {
  ...
  async function fetchFromApi() {
    ...
    try {
      ...
      setMensaje({
        tipo: "success",
        texto:
          "Tasas obtenidas de la API. Haz clic en Guardar para aplicar los cambios.",
      });
    } catch (error) {
      ...
      setMensaje({
        tipo: "error",
        texto:
          "Error al consultar la API. Puedes ingresar las tasas manualmente.",
      });
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <p className="text-slate-400 text-sm sm:text-base">
          Configura las tasas de cambio para tu negocio
        </p>
        <Button
        ...
        >
          ...
          Obtener del Banco Central de Venezuela
        </Button>
      </div>
        ...
        <CardContent>
          <div className="space-y-4">
            {tasas.map((tasa) => (
              <div
                key={tasa.moneda}
                className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 p-4 bg-slate-700/50 rounded-lg"
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-slate-400 text-sm w-20 sm:w-24 text-right shrink-0">
      ...
      <Card>
        <CardContent>
          <div className="text-sm text-slate-400 space-y-2">
            <p>
              <strong className="text-slate-300">Nota:</strong> Los precios de
              los productos se almacenan en USD (dólares).
            </p>
            <p>
              Las tasas de cambio se usan para mostrar los precios en la moneda
              que selecciones.
            </p>
            <p>
              Puedes obtener las tasas actualizadas automáticamente desde la API
              o ingresarlas manualmente.
            </p>
          </div>
        </CardContent>
      </Card>
