...
import { Users, Search, ShoppingCart, ChevronLeft, ChevronRight, Calendar, AlertCircle } from 'lucide-react'
...
export default function ClientesPage() {
  ...
  async function fetchData() {
    ...
    const [clientesRes, ventasRes] = await Promise.all([
      ...
      supabase.from('ventas').select('cliente_nombre, total, total_pagado, estado, created_at').order('created_at', { ascending: false })
    ])
    ...
    const estadisticasPorCliente: Record<string, {
      total_compras: number
    ...
    ventasData.forEach((venta: {
      ...
      estadisticasPorCliente[nombreNormalizado].total_facturado += venta.total
      estadisticasPorCliente[nombreNormalizado].total_pagado += venta.total_pagado || 0
      ...
      if (!estadisticasPorCliente[nombreNormalizado].ultima_compra) {
        estadisticasPorCliente[nombreNormalizado].ultima_compra = venta.created_at
      }
    })

    // Combinar clientes con sus estadísticas
    const clientesConEstadisticas: ClienteConEstadisticas[] = clientesData.map(cliente => {
    ...
    clientesConEstadisticas.sort((a, b) => {
      ...
      if (a.deuda_pendiente !== b.deuda_pendiente) return b.deuda_pendiente - a.deuda_pendiente
  ...
  const totalFacturado = clientes.reduce((sum, c) => sum + c.total_facturado, 0)
  const totalCobrado = clientes.reduce((sum, c) => sum + c.total_pagado, 0)
  const totalPendiente = clientes.reduce((sum, c) => sum + c.deuda_pendiente, 0)
        ...
        <Card>
          <CardContent className="py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-slate-100">{totalVentas}</p>
        ...
        <Card>
          <CardContent className="py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xl font-bold text-slate-100">{formatearPrecio(totalFacturado)}</p>
        ...
        <Card>
          <CardContent className="py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xl font-bold text-green-400">{formatearPrecio(totalCobrado)}</p>
        ...
        <Card className={totalPendiente > 0 ? 'border-red-500/50' : ''}>
          <CardContent className="py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xl font-bold text-red-400">{formatearPrecio(totalPendiente)}</p>
                <p className="text-sm text-slate-400">Pendiente ({clientesConDeuda})</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {clientesFiltrados.length === 0 ? (
        ...
        <>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700">
                  ...
                  <th className="text-center px-6 py-4 text-sm font-medium text-slate-400">Compras</th>
                  <th className="text-right px-6 py-4 text-sm font-medium text-slate-400">Facturado</th>
                  <th className="text-right px-6 py-4 text-sm font-medium text-slate-400">Pagado</th>
                  <th className="text-right px-6 py-4 text-sm font-medium text-slate-400">Pendiente</th>
                  <th className="text-right px-6 py-4 text-sm font-medium text-slate-400">Última compra</th>
                </tr>
              </thead>
              <tbody>
                      ...
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                          ...
                          <span className={`font-medium text-sm ${
                            cliente.tiene_deuda ? 'text-red-300' : 'text-slate-300'
                          }`}>
                            {cliente.nombre.charAt(0).toUpperCase()}
                          </span>
                        </div>
                        <div>
                          <p className="font-medium text-slate-100">{cliente.nombre}</p>
                          {cliente.tiene_deuda && (
          ...
          <div className="lg:hidden space-y-4">
            {clientesPaginados.map((cliente) => (
              <Card key={cliente.id} className={cliente.tiene_deuda ? 'border-red-500/50' : ''}>
                <CardContent>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-slate-100 truncate">{cliente.nombre}</p>
                  ...
                  <div className="grid grid-cols-4 gap-2 mt-4 pt-4 border-t border-slate-700">
                    <div className="text-center">
                      <p className="text-lg font-bold text-blue-400">{cliente.total_compras}</p>
